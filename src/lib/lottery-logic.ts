 ...
 */
export const phanBoGiaiFix = (totalLanQuay: number, nguoiGiaiFix: IDataUser[]): number[] => {
...
export const quayChoNguoiTrungGiai = (
  ...
  const danhSachThamGia = getDanhSachThamGia(danhSachNguoi, loaiGiai, quyTac.loaiDS);
  
  if (danhSachThamGia.length === 0) return null;
  
  // Lấy danh sách người có giải fix cho loại giải này
  const nguoiGiaiFix = danhSachThamGia.filter(user => user.GiaiFix === loaiGiai);
  ...
  if (indexGiaiFix !== -1 && nguoiGiaiFix.length > indexGiaiFix) {
    ...
    const danhSachNgauNhien = danhSachThamGia.filter(user => 
      !user.GiaiFix || user.GiaiFix !== loaiGiai
    );
 ...
 */
export const quayTatCaGiai = (danhSachNguoi: IDataUser[]): { [key: string]: IDataUser[] } => {
  ...
  for (let i = 0; i < QUAT_QUY_QUAY_SO["3"].soLanQuay; i++) {
    const nguoiTrung = quayChoNguoiTrungGiai(danhSachNguoi, "3", i, QUAT_QUY_QUAY_SO["3"].soLanQuay);
    if (nguoiTrung) {
  ...
  for (let i = 0; i < QUAT_QUY_QUAY_SO["2"].soLanQuay; i++) {
    const nguoiTrung = quayChoNguoiTrungGiai(danhSachNguoi, "2", i, QUAT_QUY_QUAY_SO["2"].soLanQuay);
    if (nguoiTrung) {
  ...
  for (let i = 0; i < QUAT_QUY_QUAY_SO["1"].soLanQuay; i++) {
    const nguoiTrung = quayChoNguoiTrungGiai(danhSachNguoi, "1", i, QUAT_QUY_QUAY_SO["1"].soLanQuay);
    if (nguoiTrung) {
  ...
  const nguoiTrungDB = quayChoNguoiTrungGiai(danhSachNguoi, "db", 0, QUAT_QUY_QUAY_SO["db"].soLanQuay);
  if (nguoiTrungDB) {
